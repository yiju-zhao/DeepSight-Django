...
class Command(BaseCommand):
    ...
    def _run_operation(
        self, operation: str, days: int, limit: int, dry_run: bool, force: bool
    ):
    ...
    @transaction.atomic
    def _cleanup_empty_notebooks(
        self, days: int, limit: int, dry_run: bool, force: bool
    ):
        ...
        empty_notebooks = Notebook.objects.filter(
            created_at__lt=cutoff_date, knowledge_base_items__isnull=True
        ).distinct()[:limit]
        ...
        if not dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(f"Deleted {count} empty notebooks"))
            logger.info(f"Deleted empty notebooks: {deleted_names}")
        else:
            for notebook in empty_notebooks:
                self.stdout.write(
                    f"Would delete: {notebook.name} (created: {notebook.created_at})"
                )
    ...
    def _cleanup_failed_jobs(self, days: int, limit: int, dry_run: bool, force: bool):
        ...
        failed_jobs = BatchJob.objects.filter(
            status__in=["failed", "error"], created_at__lt=cutoff_date
        )[:limit]
        ...
        if not dry_run:
            failed_jobs.delete()
            self.stdout.write(self.style.SUCCESS(f"Deleted {count} failed jobs"))
            logger.info(f"Deleted {count} failed batch jobs")
        else:
            for job in failed_jobs:
                self.stdout.write(
                    f"Would delete job: {job.id} ({job.job_type}, {job.status})"
                )
    ...
    def _cleanup_orphaned_files(self, limit: int, dry_run: bool, force: bool):
        ...
        orphaned_items = KnowledgeBaseItem.objects.filter(notebook__isnull=True)[:limit]
        ...
        if not dry_run:
            ...
            for item in orphaned_items:
                self.stdout.write(
                    f"Would delete orphaned item: {item.id} ({item.title})"
                )
    ...
    def _reset_stuck_processing(self, days: int, dry_run: bool, force: bool):
        ...
        stuck_items = KnowledgeBaseItem.objects.filter(
            parsing_status="failed", created_at__lt=cutoff_date
        )
        ...
        self.stdout.write(
            f"Found {count} items stuck in processing for more than {days} days"
        )
        ...
        if not dry_run:
            stuck_items.update(processing_status="failed", updated_at=timezone.now())
            self.stdout.write(
    ...
    def _optimize_database(self, dry_run: bool, force: bool):
        ...
        if not force and not dry_run:
            confirm = input(
                "Run database optimization? This may take some time [y/N]: "
            )
            if confirm.lower() != "y":
                self.stdout.write("Operation cancelled")
                return
