...
class Command(BaseCommand):
    ...
    def _backup_full(
        self, backup_dir: Path, incremental: bool, compress: bool, exclude_large: bool
    ):
        ...
        self._backup_storage(
            backup_dir / "storage", incremental, compress, exclude_large
        )
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    if file_content:
                        file_path = (
                            backup_dir / f"files/{item.id}_{item.file_object_key}"
                        )
            ...
            self.stdout.write(
                f"Storage backup completed: {len(files_manifest)} files backed up"
            )
    ...
    def _backup_vectors(self, backup_dir: Path, compress: bool):
        ...
        try:
            ...
            user_ids = set(
                KnowledgeBaseItem.objects.values_list("notebook__user_id", flat=True)
            )
            ...
            for user_id in user_ids:
                if user_id:
                    try:
                        ...
                        notebooks = Notebook.objects.filter(user_id=user_id).exclude(
                            ragflow_dataset_id=""
                        )
                        ...
                        logger.warning(
                            f"Failed to backup RagFlow data for user {user_id}: {e}"
                        )
                        continue
            ...
            self.stdout.write(
                f"Vector database metadata backed up: {len(collections_metadata)} collections"
            )
    ...
    def _create_backup_metadata(
        self, backup_dir: Path, backup_type: str, incremental: bool
    ):
