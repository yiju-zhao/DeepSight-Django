    ...
    try:
        ...
        if self.request.called_directly is False:  # Only check if running in worker
            ...
            if task_result.state == "REVOKED":
                logger.info(
                    f"Task {self.request.id} was revoked, aborting podcast generation"
                )
        ...
        language = (
            job.source_metadata.get("language", "en") if job.source_metadata else "en"
        )
        ...
        result = asyncio.run(
            podcast_service.create_podcast_with_panel_crew(
        ...
        if result["status"] == "completed":
            ...
            logger.error(
                f"Podcast generation failed for job {job_id}: {result['error']}"
            )
    ...
    try:
        ...
        if job.celery_task_id:
            try:
                ...
                logger.info(
                    f"Revoked Celery task {job.celery_task_id} for job {job_id}"
                )
            except Exception as e:
