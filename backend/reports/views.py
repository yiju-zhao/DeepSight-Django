...
from .services import JobService, PdfService
...
class ReportJobListCreateView(APIView):
    ...
    def get(self, request):
        try:
            ...
            qs = Report.objects.filter(user=request.user).exclude(
            ...
            last_modified = qs.aggregate(max_updated=models.Max("updated_at"))[
                "max_updated"
            ]
                ...
                if report.status == Report.STATUS_COMPLETED:
                    if report.main_report_object_key or report.result_content:
                        validated_reports.append(
                            ReportViewHelper.format_report_data(report)
                        )
                    else:
                        logger.warning(
            ...
            has_active_jobs = any(
                r.get("status") in ["pending", "running"] for r in validated_reports
            )
    ...
    def post(self, request):
        try:
            ...
            notebook_id = serializer.validated_data.get("notebook") or request.data.get(
                "notebook"
            )
            if not notebook_id:
                return Response(
            ...
            logger.info(f"Report {report.id} created (canonical)")
...
class ReportJobDetailView(APIView):
    ...
    def get(self, request, report_id):
        try:
            ...
            logger.error(
                f"Error getting report status (canonical) for report {report_id}: {e}"
            )
            return Response(
                {"detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        try:
            ...
            if report.main_report_object_key:
                try:
                    ...
                    logger.warning(
                        f"Could not update report file for report {report_id}: {e}"
                    )
    ...
    def delete(self, request, report_id):
        ...
        try:
            ...
            logger.info(
                f"DELETE request for report {report_id} (status: {report.status})"
            )
            ...
            logger.error(
                f"Unexpected error during report deletion report {report_id}: {e}"
            )
            return Response(
                {
                    "success": False,
    ...
    def get(self, request, report_id):
        try:
            report = get_object_or_404(
                Report.objects.filter(user=request.user), id=report_id
            )
            ...
            if filename:
                if (
                    report.main_report_object_key
                    and report.file_metadata.get("main_report_filename") == filename
                ):
            ...
            logger.error(
                f"Error downloading report (canonical) for report {report_id}: {e}"
            )
            return Response(
                {"detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        try:
            ...
            if report.result_content:
                ...
                try:
                    ...
                    content_bytes = storage_adapter.get_file_content(
                        report.main_report_object_key, str(request.user.id)
                    )
                    ...
                    logger.error(
                        f"Error reading report file for report {report_id}: {e}"
                    )
            ...
            try:
                ...
                logger.error(
                    f"Error converting report to PDF for report {report_id}: {e}"
                )
            ...
            logger.error(
                f"Error downloading PDF report (canonical) for report {report_id}: {e}"
            )
            return Response(
                {"detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ReportJobContentView(APIView):
    ...
    def get(self, request, report_id):
        try:
            ...
            logger.error(
                f"Error getting report content (canonical) for report {report_id}: {e}"
            )
            return Response(
                {"detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ReportJobImageInlineView(APIView):
    ...
    def get(self, request, report_id, image_id):
        ...
        try:
            ...
            logger.exception(
                f"Failed to serve inline image {image_id} for report {report_id}: {e}"
            )
...
class ReportJobCancelView(APIView):
    ...
    def post(self, request, report_id):
        ...
        try:
            ...
            logger.info(
                f"Cancelling report {report_id} (status: {report.status}, celery_task_id: {report.celery_task_id})"
            )
            ...
            if report.celery_task_id:
                try:
                    ...
                    celery_app.control.revoke(
                        report.celery_task_id, terminate=True, signal="SIGKILL"
                    )
                    logger.info(
                        f"Sent SIGKILL to Celery task {report.celery_task_id} for immediate termination"
                    )

                except Exception as e:
                    logger.error(
                        f"Failed to revoke Celery task {report.celery_task_id}: {e}"
                    )
