...
class GradeCompleteness(BaseModel):
    ...
    new_docs_contribute: bool = Field(
        description="Whether newly retrieved documents contribute to completing the story (True if no new docs)"
    )
    is_complete: bool = Field(
        description="The collective documents are sufficient to answer the original question"
    )
    missing_info: str | None = Field(
        description="Description of what information is still missing if not complete"
    )
    search_advice: str | None = Field(
        description="Specific advice on what to search for next if incomplete"
    )
...
class SemanticGroup(BaseModel):
    ...
    description: str = Field(
        description="Short description of what these chunks collectively cover"
    )
    chunk_ids: list[int] = Field(
        description="Ordered list of chunk IDs belonging to this group"
    )
...
class ReorderedContext(BaseModel):
    ...
    groups: list[SemanticGroup] = Field(
        description="List of semantic groups containing ordered chunk IDs"
    )
...
class DeepSightRAGAgent:
    ...
    def _initialize_models(self):
        ...
        logger.info(
            f"Initializing models - Nano: {self.config.nano_model_name}, Full: {self.config.model_name}"
        )
    ...
    async def initialize_request(
        ...
        if last_human_message:
            ...
            await adispatch_custom_event(
                "manually_emit_state", updated_state, config=config
            )
    ...
    async def planning(self, state: RAGAgentState, config: RunnableConfig) -> dict:
        ...
        prompt = format_planning_prompt(
            question=question,
            previous_queries=previous_queries if iteration_count > 0 else None,
        )

        response = await self.response_model.ainvoke(
        ...
        # Emit state update to frontend
        await adispatch_custom_event(
            "manually_emit_state", {**state, **updated_state}, config=config
        )
    ...
    async def retrieve(self, state: RAGAgentState, config: RunnableConfig) -> dict:
        ...
        if tools:
            retrieval_tool = next(
                (
                    t
                    for t in tools
                    if "retrieval" in t.name.lower() or "search" in t.name.lower()
                ),
                None,
            )
            if retrieval_tool:
                for q in queries:
                    try:
                        ...
                        logger.error(
                            f"Error calling retrieval tool for query '{q}': {e}"
                        )
        ...
        logger.info(
            f"Retrieved {len(unique_new_docs)} unique documents from {len(queries)} queries"
        )

        updated_state = {"new_documents": unique_new_docs, "current_step": "retrieving"}
    ...
    async def grade_relevance(
        ...
        for d in new_documents:
            ...
            prompt = format_grade_documents_prompt(
                question=original_question, context=d
            )
            score = await structured_grader.ainvoke(
                [HumanMessage(content=prompt)], config
            )
        ...
        logger.info(
            f"---FILTERED {len(relevant_new_docs)} NEW RELEVANT DOCS. TOTAL: {len(all_relevant_docs)}---"
        )
    ...
    async def reorder(self, state: RAGAgentState, config: RunnableConfig) -> dict:
        ...
        prompt = format_reorder_prompt(
            question=original_question, context=formatted_context
        )

        # 2. Call LLM with structured output
        structured_reorderer = self.grader_model.with_structured_output(
            ReorderedContext
        )
        result = await structured_reorderer.ainvoke(
            [HumanMessage(content=prompt)], config
        )
        ...
        logger.info(
            f"Reordering complete. Created {len(final_groups)} semantic groups."
        )

        updated_state = {"semantic_groups": final_groups, "current_step": "reordering"}
    ...
    async def prepare_generation(
        ...
        updated_state = {"current_step": "synthesizing", "synthesis_progress": 0}
    ...
    def decide_after_relevance(
    ...
    ) -> Literal["planning", "reorder", "generate"]:
        """
        ...
        if iteration_count < 3:
            logger.info(
                f"---DECISION: NO RELEVANT DOCS (Iter {iteration_count}/3), RE-PLANNING---"
            )
            return "planning"

        logger.info(
            "---DECISION: NO RELEVANT DOCS AFTER 3 ITERS, FORCING GENERATION (Failure message)---"
        )
        return "generate"
    ...
    async def grade_generation_v_documents_and_question(
        ...
        if iteration_count >= self.config.max_iterations:
            logger.warning(
                f"---ITERATION LIMIT REACHED ({iteration_count}/{self.config.max_iterations}), ACCEPTING GENERATION---"
            )
            return "useful"
        ...
        # Check hallucination
        hallucination_grader = self.grader_model.with_structured_output(
            GradeHallucinations
        )
