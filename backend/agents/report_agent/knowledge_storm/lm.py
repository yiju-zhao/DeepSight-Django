...
class DeepSeekModel(dspy.OpenAI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prompt_tokens = 0
        self.completion_tokens = 0
        self.model = gpt_model  # Keep for OpenAI client compatibility
    ...
    def get_usage_and_reset(self):
        """Get the total tokens used and reset the token usage."""
        usage = {
            self.kwargs["model"]: {  # Use the overridden model name
                "prompt_tokens": self.prompt_tokens,
                "completion_tokens": self.completion_tokens,
            }
        }
    ...
    def _create_completion(self, prompt: str, **kwargs):
        ...
        data = {
            "model": self._deepseek_model,  # Use the actual DeepSeek model name
