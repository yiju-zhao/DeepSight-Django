...
class ArticleTextProcessing:
    ...
    def clean_up_citation(conv):
        for turn in conv.dlg_history:
            # Skip if agent_utterance is None
            if turn.agent_utterance is None:
                continue
    ...
    def clean_up_outline(outline, topic=""):
        ...
        # Skip the first line if it appears to be a title (i.e., has only a single #)
        # This avoids hardcoding specific keywords
        skip_first_line = False
        if lines and lines[0].startswith("# ") and lines[0].count("#") == 1:
            # Check if there are other lines with single # - if so, this could be a legitimate first section
            ...
            # If this is the only first-level heading, assume it's a title and skip it
            if not has_other_first_level and len(lines) > 1:
                skip_first_line = True

        for i, line in enumerate(outline.split("\n")):
            # Skip the title line if identified
...
class QueryLogger:
    ...
    This logger creates a clean, hierarchical structure where each query and its
    retrieval steps are stored together in a single JSON object. The structure is:

    {
        "query_id": "query_1",
        "queries": ["search query"],
        "retrieval_steps": {
            "initial_vector": [{"title": "Doc1", "url": "http://..."},...],
            "initial_bm25": [{"title": "Doc2", "url": "http://..."},...],
            "fusion": [{"title": "Doc1", "url": "http://...", "score": 0.95},...],
            "rerank": [{"title": "Doc1", "url": "http://...", "score": 0.98},...],
            "final": [{"title": "Doc1", "url": "http://..."},...]
        }
    }

    Each query is logged as a complete unit, making it easy to analyze the retrieval
    ...
    def log(self, query_data: dict):
        """
        Atomically logs the complete retrieval data for a single query.
        Handles query ID generation and thread-safe file writing.

        Args:
            query_data (dict): A dictionary containing the 'queries' list and
                               a 'retrieval_steps' dictionary populated with
                               all stages (initial_vector, initial_bm25, fusion,
                               rerank, final).
        """
        try:
