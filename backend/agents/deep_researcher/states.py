...
class SourceInfo(BaseModel):
    ...
    relevance_score: float | None = Field(
        default=None, description="Relevance score (0-1) if available"
    )
...
class ResearchResult(BaseModel):
    ...
    findings: str = Field(description="Compressed research summary with key insights")
    sources: list[SourceInfo] = Field(
        default_factory=list, description="List of cited sources with URLs and snippets"
    )
    ...
    research_brief: str | None = Field(
        default=None, description="Original research query/brief"
    )
