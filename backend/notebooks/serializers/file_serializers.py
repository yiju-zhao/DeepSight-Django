...
from ..utils.helpers import check_content_duplicate, check_source_duplicate
...
class FileUploadSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if request and hasattr(request, "user") and request.user.is_authenticated:
            ...
            is_pasted_text = (
                original_filename.endswith(".md")
                and file.content_type in ["text/markdown", "text/plain"]
                and file.size < 50000  # Reasonable limit for pasted text
            )
            ...
            logger.info(
                f"[FileUploadSerializer] Validating file: {original_filename}, is_pasted_text: {is_pasted_text}, content_type: {file.content_type}, size: {file.size}"
            )
            ...
            if is_pasted_text:
                ...
                try:
                    ...
                    existing_item = check_content_duplicate(
                        file_content, user_id, notebook_id
                    )
                    if existing_item:
                        raise serializers.ValidationError(
                            {
                                "file": "This text content already exists in your workspace. Duplicate content detected.",
                    ...
                    logger = logging.getLogger(__name__)
                    logger.warning(
                        f"Could not read file content for duplicate check: {e}"
                    )

            # For regular files, check source duplicate using original filename
            existing_item = check_source_duplicate(
                original_filename, user_id, notebook_id
            )
            if existing_item:
                raise serializers.ValidationError(
                    {
...
class VideoImageExtractionSerializer(serializers.Serializer):
    ...
    video_file_id = serializers.CharField(
        max_length=64, help_text="Video file ID in format f_{file_id}"
    )
    ...
    min_words = serializers.IntegerField(
        default=20, min_value=0, help_text="Minimum words per caption (default: 20)"
    )
    max_words = serializers.IntegerField(
        default=100, min_value=0, help_text="Maximum words per caption (default: 100)"
    )
...
class BatchFileUploadSerializer(serializers.Serializer):
    ...
    files = serializers.ListField(
        child=serializers.FileField(), required=False, allow_empty=False
    )
    upload_file_id = serializers.CharField(required=False)
