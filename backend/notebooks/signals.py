...
def delete_image_file_on_pre_delete(
    sender, instance: KnowledgeBaseImage, using, **kwargs
):
    ...
    try:
        ...
        if instance.ragflow_document_id and instance.notebook.ragflow_dataset_id:
            ...
            try:
                ...
                if success:
                    logger.info(
                        f"Successfully deleted RagFlow document '{instance.title}' (ID: {instance.ragflow_document_id}) from dataset {instance.notebook.ragflow_dataset_id}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Successfully updated RagFlow dataset {instance.notebook.ragflow_dataset_id} after document deletion"
                        )
                    except Exception as update_error:
                        # Log error but don't fail the deletion process
                        logger.warning(
                            f"Failed to update dataset {instance.notebook.ragflow_dataset_id} after deletion: {update_error}"
                        )
                else:
                    logger.warning(
                        f"RagFlow document deletion returned False for document {instance.ragflow_document_id}"
                    )
            except Exception as e:
                logger.error(
                    f"Failed to delete RagFlow document {instance.ragflow_document_id} ('{instance.title}') from dataset {instance.notebook.ragflow_dataset_id}: {e}"
                )
