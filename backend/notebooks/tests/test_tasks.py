...
from ..models import BatchJob, BatchJobItem, Notebook
from ..tasks import (
...
class TaskTests(TestCase):
    ...
    def test_check_batch_completion(self):
        ...
        batch_job = BatchJob.objects.create(
            notebook=self.notebook, job_type="url_parse", total_items=3
        )
    ...
    def test_cleanup_old_batch_jobs(self):
        ...
        old_job = BatchJob.objects.create(
            notebook=self.notebook, job_type="url_parse", status="completed"
        )
        ...
        recent_job = BatchJob.objects.create(
            notebook=self.notebook, job_type="file_upload", status="completed"
        )
