...
class FileViewSetTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.other_notebook = Notebook.objects.create(
            user=self.other_user, name="Other Notebook"
        )
    ...
    def test_raw_action_returns_attachment(self, mock_kb_service):
        ...
        self.assertEqual(
            response["Content-Disposition"], 'attachment; filename="test_file.pdf"'
        )
    ...
    def test_inline_action_returns_inline(self, mock_kb_service):
        ...
        self.assertEqual(response["Content-Type"], "application/pdf")
        self.assertEqual(
            response["Content-Disposition"], 'inline; filename="test_file.pdf"'
        )
        ...
        for case in test_cases:
            with self.subTest(content_type=case["content_type"]):
                ...
                self.assertEqual(
                    ...
                    f'inline; filename="{case["filename"]}"',
                )
                self.assertEqual(response["X-Content-Type-Options"], "nosniff")
