...
class TaskOptionsSerializer(serializers.Serializer):
    ...
    timeout = serializers.FloatField(
        min_value=60, max_value=1800,  # 1 min to 30 min
        required=False
    )
...
class ExecuteTaskSerializer(serializers.Serializer):
    ...
    goal = serializers.CharField(
        max_length=5000,
        help_text="The user's research goal or question"
    )
...
class ClarificationResponseSerializer(serializers.Serializer):
    ...
    response = serializers.CharField(
        max_length=2000,
        help_text="User's response to clarification questions"
    )
...
class StudioExecuteSSEView(View):
    ...
    def post(self, request, notebook_id: str):
        ...
        try:
            ...
            logger.info(f"Starting task {task_id} for notebook {notebook.id}: {goal[:100]}")
            ...
            try:
                ...
                try:
                    result = loop.run_until_complete(
                        execute_task(task.goal, options)
                    )
                finally:
                    loop.close()
                ...
                if result.status.value == 'completed':
                    ...
                    yield sse_event('result', {
                        'task_id': task.task_id,
                        'findings': result.findings[:500] if result.findings else None,
                        'report_preview': result.final_report[:1000] if result.final_report else None,
