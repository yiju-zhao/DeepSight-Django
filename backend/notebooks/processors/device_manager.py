...
class DeviceManager:
    ...
    def get_device_info_detailed(self) -> dict:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                device_info["memory_info"] = {
                    "note": "MPS memory info not available via PyTorch API"
                }
        except ImportError:
            pass
    ...
    def setup_device_environment(self, device_type: str, gpu_id: int | None = None):
        """
        ...
        if device_type == "cuda":
            if gpu_id is not None:
                ...
                self.log_operation(
                    "cuda_device_setup", f"Set CUDA_VISIBLE_DEVICES to {gpu_id}"
                )
            ...
            self.log_operation(
                "mps_device_setup", "Configured environment for MPS acceleration"
            )
        else:
            # CPU mode
            os.environ["TORCH_DEVICE"] = "cpu"
            self.log_operation(
                "cpu_device_setup", "Configured environment for CPU processing"
            )
