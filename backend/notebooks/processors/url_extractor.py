...
class URLExtractor:
    ...
    async def _load_crawl4ai(self):
        ...
        try:
            ...
            self.log_operation(
                "crawl4ai_import_error", f"crawl4ai not available: {e}", "warning"
            )
            self._crawl4ai_loaded = False
    ...
    async def _check_media_availability(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.log_operation(
                "media_check_error",
                f"Error checking media availability for {url}: {e}",
                "error",
            )
    ...
    async def _extract_with_crawl4ai(
        ...
        try:
            ...
            self.log_operation(
                "crawl4ai_extract_error",
                f"Error extracting with crawl4ai from {url}: {e}",
                "error",
            )
            raise

    async def _download_and_transcribe_media(
        self, url: str, media_info: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.log_operation(
                ...
                f"Error downloading/transcribing media: {e}",
                "error",
            )
            raise
        finally:
        ...
        try:
            ...
            for file_path in Path(temp_dir).iterdir():
                if file_path.is_file() and base_filename in file_path.name:
                    self.log_operation(
                        "video_download", f"Downloaded video: {file_path}"
                    )
            ...
            self.log_operation(
                "video_download_error", f"Error downloading video: {e}", "error"
            )
            return None
    ...
    async def _download_audio(
        ...
        try:
            ...
            for file_path in Path(temp_dir).iterdir():
                if file_path.is_file() and base_filename in file_path.name:
                    self.log_operation(
                        "audio_download", f"Downloaded audio: {file_path}"
                    )
                    return str(file_path)
            ...
            self.log_operation(
                "audio_download_error", f"Error downloading audio: {e}", "error"
            )
            return None
    ...
    async def process_url(
        ...
        try:
            ...
            cleaned_content = self._clean_markdown_content(
                features.get("content", ""), features
            )
            ...
            self.log_operation(
                "process_url_error", f"Error processing URL {url}: {e}", "error"
            )
            raise

    async def process_url_with_media(
        self,
        url: str,
        extraction_options: dict[str, Any] | None = None,
        upload_url_id: str | None = None,
        ...
        try:
            ...
            if original_file_path and os.path.exists(original_file_path):
                try:
                    ...
                    if (
                        ...
                        and "deepsight_media_" in temp_dir
                    ):
                        import shutil
                    ...
                    self.log_operation(
                        "cleanup_error",
                        f"Failed to cleanup temp files: {cleanup_err}",
                        "warning",
                    )
            ...
            self.log_operation(
                "process_url_with_media_error",
                f"Error processing URL with media {url}: {e}",
                "error",
            )
            raise

    async def process_url_media_only(
        self,
        url: str,
        upload_url_id: str | None = None,
        user_id: int = None,
        ...
        try:
            ...
            if not media_result.get("success"):
                raise Exception(
                    f"Media processing failed: {media_result.get('error', 'Unknown error')}"
                )
            ...
            self.log_operation(
                "process_url_media_only_error",
                f"Error processing URL media only {url}: {e}",
                "error",
            )
            raise

    async def process_url_document_only(
        self,
        url: str,
        upload_url_id: str | None = None,
        user_id: int = None,
        ...
        try:
            ...
            self.log_operation(
                "document_download_start", f"Starting document download from {url}"
            )
            ...
            if kb_item_id:
                # Update existing KB item
                await self._process_and_update_document_file(
                    temp_file_path, file_info, url, kb_item_id, user_id, notebook_id
                )
                file_id = kb_item_id
                processed_result = {"file_id": kb_item_id, "status": "completed"}
            else:
                # Create new KB item
                processed_result = await self._process_document_file(
                    temp_file_path, file_info, url, upload_url_id, user_id, notebook_id
                )
            ...
            self.log_operation(
                "process_url_document_error",
                f"Error processing document URL {url}: {e}",
                "error",
            )
            raise
        finally:
            ...
            if temp_file_path and os.path.exists(temp_file_path):
                try:
                    ...
                    self.log_operation(
                        "temp_cleanup", f"Cleaned up temporary file: {temp_file_path}"
                    )
                except Exception as cleanup_error:
                    self.log_operation(
                        "temp_cleanup_error",
                        f"Error cleaning up {temp_file_path}: {cleanup_error}",
                        "warning",
                    )
    ...
    async def _download_document_to_temp(self, url: str) -> str:
        ...
        try:
            ...
            if not filename:
                ...
                self.log_operation(
                    "debug_filename_fallback",
                    f"Empty filename, using fallback: {filename}",
                )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    self.log_operation(
                        "download_info", f"Content-Type: {content_type}, URL: {url}"
                    )
            ...
            self.log_operation(
                "download_error", f"Error downloading document from {url}: {e}", "error"
            )
            raise
        ...
        try:
            ...
            if correct_extension:
                ...
                if not current_extension or current_extension != correct_extension:
                    ...
                    self.log_operation(
                        "extension_fixed",
                        f"Fixed extension from {current_extension} to {correct_extension}",
                    )
                    return str(new_path)
            ...
            self.log_operation(
                "extension_fix_error", f"Error fixing extension: {e}", "warning"
            )
            return file_path
    ...
    async def _validate_document_format(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.log_operation(
                "file_validation",
                f"File: {filename}, MIME: {mime_type}, Type: {file_type}",
            )
            ...
            self.log_operation(
                "validation_error", f"Error validating file format: {e}", "error"
            )
            raise
    ...
    async def _process_document_file(
        ...
        try:
            ...
            self.log_operation(
                ...
                f"Error processing document file: {e}",
                "error",
            )
            raise

    async def _process_and_update_document_file(
        self,
        file_path: str,
        file_info: dict,
        url: str,
        ...
        try:
            ...
            await self.upload_processor.process_upload(
                django_file,
            ...
            self.log_operation(
                "document_processed",
                f"Successfully processed and updated document from URL: {url}",
            )

        except Exception as e:
            self.log_operation(
                "document_update_error",
                f"Error processing and updating document file: {e}",
                "error",
            )
            raise
        ...
        try:
            ...
            if original_file_path and processing_type == "media":
                ...
                url_metadata = {
                    ...
                    "content_type": self._get_mime_type_from_extension(
                        os.path.splitext(original_basename)[1]
                    ),
                ...
                url_metadata = {
                    ...
                    "processing_metadata": {
                        "extraction_type": "url_extractor",
                        "extraction_success": True,
                        "extraction_method": features.get(
                            "extraction_method", "crawl4ai"
                        ),
                        "content_length": len(content),
                        "processing_type": processing_type,
            ...
            store_file_sync = sync_to_async(
                self.storage_adapter.store_processed_file, thread_sensitive=False
            )
            ...
            self.log_operation(
                "store_content", f"Stored URL content with file_id: {file_id}"
            )
            return file_id

        except Exception as e:
            self.log_operation(
                "store_content_error", f"Error storing URL content: {e}", "error"
            )
            raise

    async def _update_existing_kb_item(
        self,
        kb_item_id: str,
        url: str,
        content: str,
        ...
        try:
            ...
            self.log_operation(
                "update_kb_item",
                f"Updated existing KB item {kb_item_id} with URL content using minio storage",
            )

        except Exception as e:
            self.log_operation(
                "update_kb_item_error",
                f"Error updating existing KB item {kb_item_id}: {e}",
                "error",
            )
            raise
