...
class FileTypeProcessors:
    ...
    def check_mineru_health(self) -> bool:
        ...
        try:
            ...
            self.log_operation(
                "mineru_health_check_failed",
                f"MinerU API health check failed: {e}",
                "warning",
            )
            return False
    ...
    def process_pdf_mineru(self, file_path: str, file_metadata: dict) -> dict[str, Any]:
        ...
        try:
            self.log_operation(
                "pdf_mineru_start", f"Starting MinerU processing of {file_path}"
            )
            ...
            if not self.check_mineru_health():
                self.log_operation(
                    "pdf_mineru_unavailable",
                    "MinerU API not available, falling back to PyMuPDF",
                )
            ...
            self.log_operation(
                "pdf_mineru_files",
                f"Created {len(os.listdir(temp_dir))} files: {[os.path.basename(f) for f in [md_file_path] + image_files]}",
            )
            ...
            self.log_operation(
                "pdf_mineru_completed",
                f"MinerU processing completed in {duration:.2f} seconds",
            )
            ...
            self.log_operation(
                "pdf_mineru_error", f"MinerU processing failed: {e}", "warning"
            )
            # Fallback to PyMuPDF if MinerU fails
            return self.process_pdf_pymupdf_fallback(file_path, file_metadata)

    def process_pdf_pymupdf_fallback(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
        """Fallback PDF text extraction using PyMuPDF when MinerU is not available."""
        try:
            self.log_operation(
                "pdf_pymupdf_fallback", f"Using PyMuPDF fallback for {file_path}"
            )
    ...
    def _call_mineru_api(
        ...
        try:
            ...
            self.log_operation(
                "mineru_api_request_failed", f"MinerU API request failed: {e}", "error"
            )
            raise Exception(f"MinerU API request failed: {e}")
        except json.JSONDecodeError as e:
            self.log_operation(
                "mineru_api_response_error",
                f"Failed to parse MinerU API response: {e}",
                "error",
            )
            raise Exception(f"Invalid JSON response from MinerU API: {e}")
        except Exception as e:
            self.log_operation(
                "mineru_api_call_error", f"MinerU API call failed: {e}", "error"
            )
            raise Exception(f"MinerU API call failed: {e}")

    async def process_audio_immediate(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
        ...
        try:
            ...
            transcription_service = TranscriptionService(
                self.xinference_url, self.model_uid, clean_title, self.logger
            )
            (
                transcript_content,
                transcript_filename,
            ) = await transcription_service.transcribe_audio_video(
    ...
    async def process_video_immediate(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
    ...
    def process_markdown_direct(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
        """Process markdown files directly, returning the original file without additional processing."""
        try:
            self.log_operation(
                "markdown_direct_processing",
                f"Processing markdown file directly: {file_path}",
            )
            ...
            self.log_operation(
                "markdown_direct_error",
                f"Error processing markdown file directly: {e}",
                "error",
            )
            raise Exception(f"Markdown processing failed: {str(e)}")

    def process_text_immediate(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
        ...
        try:
            ...
            is_pasted_text = (
                file_metadata.get("source_type") == "pasted_text"
                or file_metadata.get("original_filename") == "pasted_text.md"
            )
    ...
    def process_presentation_immediate(
        self, file_path: str, file_metadata: dict
    ) -> dict[str, Any]:
