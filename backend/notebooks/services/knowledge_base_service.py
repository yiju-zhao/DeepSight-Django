...
from ..models import BatchJob, KnowledgeBaseImage, KnowledgeBaseItem
...
class KnowledgeBaseService(NotebookBaseService):
    ...
    def get_processed_content(
        self, kb_item: KnowledgeBaseItem, expires: int = 3600
    ) -> str:
        """
        ...
        try:
            ...
            if kb_item.content and kb_item.content.strip():
                logger.debug(
                    f"Returning content from database field for KB item {kb_item.id}"
                )
                # Replace local image paths with presigned URLs for preview rendering
                return self._replace_local_image_paths(
                    kb_item.content, kb_item, expires=expires
                )
            ...
            logger.error(
                f"Error getting processed content for KB item {kb_item.id}: {e}"
            )
            return ""

    def _replace_local_image_paths(
        self, markdown: str, kb_item: KnowledgeBaseItem, expires: int = 3600
    ) -> str:
        """
        ...
        try:
            ...
            ).only("id", "minio_object_key", "image_metadata")
            ...
            logger.exception(
                f"Error replacing image URLs for KB item {kb_item.id}: {e}"
            )
            return markdown
    ...
    def get_user_knowledge_base(
        ...
        try:
            ...
            self.logger.exception(
                f"Failed to retrieve knowledge base for user {user_id}: {e}"
            )
            return {
    ...
    def link_knowledge_item_to_notebook(
        self, kb_item_id: str, notebook, user_id: int, notes: str = ""
    ) -> dict:
        """
        ...
        try:
            ...
            self.logger.exception(
                f"Failed to link KB item {kb_item_id} to notebook {notebook.id}: {e}"
            )
            return {
    ...
    def delete_knowledge_base_item(self, kb_item_id, user_id):
        ...
        try:
            ...
            if success:
                return {"success": True, "status_code": status.HTTP_204_NO_CONTENT}
            else:
                return {
            ...
            logger.exception(
                f"Failed to delete KB item {kb_item_id} for user {user_id}: {e}"
            )
            return {
    ...
    def get_knowledge_base_images(self, file_id, notebook):
        ...
        try:
            ...
            kb_item = get_object_or_404(
                KnowledgeBaseItem, id=file_id, notebook=notebook
            )
            ...
            for image in images:
                ...
                if image_url:
                    ...
                    if (
                        ...
                        and "original_filename" in image.image_metadata
                    ):
                    ...
                    image_data.append(
                        {
                            ...
                            "name": str(
                                image.figure_id
                            ),  # Use figure_id as name for API compatibility
    ...
    def get_images(self, kb_item: KnowledgeBaseItem) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(
                f"Failed to retrieve images list for KB item {kb_item.id}: {e}"
            )
            return []
    ...
    def get_batch_job_status(self, batch_job_id, notebook):
        ...
        try:
            ...
            items = BatchJobItem.objects.filter(batch_job=batch_job).order_by(
                "created_at"
            )
    ...
    def get_images_for_knowledge_base_item(
        self, kb_item_id: int, user_id: int = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            if not kb_item:
                logger.warning(
                    f"Knowledge base item {kb_item_id} not found or access denied"
                )
                return []
            ...
            logger.info(
                f"Retrieved {len(figure_data)} images for knowledge base item {kb_item_id}"
            )
            return figure_data
    ...
    def get_combined_figure_data_for_files(
        self, file_ids: list[str], user_id: int = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            for file_id in file_ids:
                ...
                clean_file_id = (
                    file_id.replace("f_", "") if file_id.startswith("f_") else file_id
                )

                # Get images for this file
                file_images = self.get_images_for_knowledge_base_item(
                    clean_file_id, user_id
                )
            ...
            logger.info(
                f"Combined figure data from {len(file_ids)} files: {len(combined_figure_data)} total images"
            )
    ...
    def update_image_caption(
        self, figure_id: str, caption: str, user_id: int = None
    ) -> bool:
        """
        ...
        try:
            ...
            if user_id:
                ...
                image_query = image_query.filter(
                    knowledge_base_item__notebook__user=user_id
                )
    ...
    def update_images_from_figure_data(
        self, kb_item_id: int, figure_data: list[dict[str, Any]], user_id: int = None
    ) -> bool:
        """
            ...
            with transaction.atomic():
                for figure in figure_data:
                    ...
                    if matching_image:
                        ...
                        logger.debug(
                            f"Updated image {matching_image.id} with caption from figure_data"
                        )

            logger.info(
                f"Updated {updated_count} images from figure_data for kb_item {kb_item_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error updating images from figure_data for kb_item {kb_item_id}: {e}"
            )
            return False

    def migrate_figure_data_json_to_database(
        self, kb_item_id: int, figure_data_path: str, user_id: int = None
    ) -> bool:
        """
        ...
        try:
            ...
            with open(figure_data_path, encoding="utf-8") as f:
            ...
            success = self.update_images_from_figure_data(
                kb_item_id, figure_data, user_id
            )

            if success:
                logger.info(
                    f"Successfully migrated figure_data.json to database for kb_item {kb_item_id}"
                )
    ...
    def delete_image(
        self, figure_id: str, user_id: int = None, delete_from_minio: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            if delete_from_minio and object_key:
                try:
                    ...
                    logger.warning(
                        f"Failed to delete image file from MinIO {object_key}: {e}"
                    )
    ...
    def get_image_url(
        self, figure_id: str, user_id: int = None, expires: int = 3600
    ) -> str | None:
        """
    ...
    def get_image_stats_for_knowledge_base_item(
        self, kb_item_id: int, user_id: int = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            stats = {
                ...
                "content_types": list(
                    images.values_list("content_type", flat=True).distinct()
                ),
            }
    ...
    def auto_populate_captions_from_content(
        self, kb_item_id: int, user_id: int = None
    ) -> bool:
        """
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Auto-populated captions for KB item {kb_item_id} from content"
                )
            ...
            logger.error(
                f"Error auto-populating captions for KB item {kb_item_id}: {e}"
            )
            return False
    ...
    def _get_markdown_content(self, kb_item):
        ...
        try:
            ...
            logger.error(
                f"Error getting markdown content for KB item {kb_item.id}: {e}"
            )
            return None

    def get_raw_file(
        self, kb_item: KnowledgeBaseItem, user_id: int = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if kb_item.original_file_object_key:
                ...
                file_data = self.storage_adapter.get_file_content(
                    kb_item.original_file_object_key, str(user_id)
                )
            ...
            # If no original file, try processed file
                ...
                # Use infrastructure storage adapter for direct MinIO access
                file_data = self.storage_adapter.get_file_content(
                    kb_item.file_object_key, str(user_id)
                )
