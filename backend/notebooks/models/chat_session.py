...
class ChatSession(BaseModel):
    ...
    session_metadata = models.JSONField(
        blank=True,
        null=True,
        help_text="Additional session configuration and metadata"
    )
    
    # Timestamps
    last_activity = models.DateTimeField(
        auto_now=True,
        help_text="Last time there was activity in this session"
    )
    started_at = models.DateTimeField(
        auto_now_add=True,
        help_text="When this session was started"
    )
    ended_at = models.DateTimeField(
        blank=True,
        null=True,
        help_text="When this session was closed/ended"
    )
    ...
    def clean(self):
        ...
        if self.session_metadata is not None and not isinstance(self.session_metadata, dict):
...
class SessionChatMessage(BaseModel):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                name='session_chat_message_not_empty'
            )
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.message_order and self.session:
            last_message = SessionChatMessage.objects.filter(session=self.session).order_by('-message_order').first()
